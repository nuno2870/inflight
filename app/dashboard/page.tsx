...
import { Menu, X, Plane, Map as MapIcon, Coffee, Languages, LogOut, Newspaper, ShoppingBag, Utensils, Navigation, Footprints, Bus, Clock, Target, ArrowRight, ChevronDown, Check, Volume2, VolumeX, User } from "lucide-react"; 
...
export default function Dashboard() {
  ...
  const handleSaveName = async (e: React.FormEvent) => {
      ...
      if (!error && data.user) { setUser(data.user); setShowNameModal(false); }
  };
  ...
  // --- PESQUISA ATUALIZADA (VAI AO BACKEND PYTHON) ---
  const handleSearch = async (data: any) => {
    setIsLoadingSearch(true);
    try {
        // Chama o API do Backend em vez de simular
  ...
  const { allLocations, selectGroups } = useMemo(() => {
      ...
      for (let i = 1; i <= 13; i++) {
          const id = `Gate${i}`;
          fullGraph[id] = { x: 55 + (i * 3.0), y: 25, label: `${t.loc_gate} ${i}`, term: 1, type: "gate", link: `Hall_N_${i}` };
      }
      for (let i = 14; i <= 26; i++) {
          const id = `Gate${i}`;
          const idx = i - 13;
          fullGraph[id] = { x: 55 + (idx * 3.0), y: 75, label: `${t.loc_gate} ${i}`, term: 1, type: "gate", link: `Hall_S_${idx}` };
      }
      Object.keys(fullGraph).forEach(key => { if (fullGraph[key].translationKey) { /* @ts-ignore */ fullGraph[key].label = t[fullGraph[key].translationKey]; } });
  ...
  const calculateRoute = () => {
    ...
    if (startNode.term !== endNode.term) {
        type = "transfer";
        const shuttleStop = graph["ShuttleStop"];
        if (startNode.term === 2) { 
            ...
            voiceMsg = language === "PT" ? "Atenção. Precisas de mudar de terminal. Segue para a paragem do autocarro." : "Attention. Terminal change required. Please proceed to the shuttle bus stop.";
        } else { 
            ...
            voiceMsg = language === "PT" ? "Vai para a paragem do shuttle lá fora." : "Please go to the shuttle stop outside.";
        }
        ...
        if (startNode.x > 38) {
             const currentCorridorY = startNode.y < 50 ? 30 : 70;
             pathD += ` L ${startNode.x},${currentCorridorY} L 38,${currentCorridorY}`;
        } else { pathD += ` L 38,${startNode.y}`; }
        const targetCorridorY = endNode.y < 50 ? 30 : 70;
        pathD += ` L 38,${targetCorridorY} L ${endNode.x},${targetCorridorY} L ${endNode.x},${endNode.y}`;
        pathString = pathD;
        ...
        voiceMsg = language === "PT"
            ? `Rota calculada. ${realMinutes} minutos a pé até ${endNode.label}.`
            : `Route calculated. It's a ${realMinutes} minute walk to ${endNode.label}.`;
    }
  ...
  const airportShops = [
      { name: "McDonald's", type: "Food", loc: "T1, Piso 2", status: t.open, color: "text-yellow-400" },
      { name: "Starbucks", type: "Coffee", loc: "T1, Portão 14", status: t.open, color: "text-green-400" },
      { name: "Duty Free", type: "Shop", loc: "T1, Central", status: t.open, color: "text-blue-400" },
      { name: "TAP Lounge", type: "Lounge", loc: "T1, Piso 3", status: t.open, color: "text-green-600" },
      { name: "Pizza Hut", type: "Food", loc: "T1, Piso 2", status: t.closed, color: "text-red-500" },
  ];

  if (loadingAuth) return <div className="min-h-screen bg-black flex items-center justify-center text-white animate-pulse">A carregar perfil...</div>;

  return (
    <main className="min-h-screen bg-black text-white relative flex overflow-x-hidden">
      {showNameModal && (
        <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
            <div className="bg-gray-900 border border-gray-700 p-8 rounded-3xl max-w-md w-full shadow-2xl animate-fade-in-up text-center">
                <div className="flex justify-center mb-4 text-blue-500"><User size={48} /></div>
                <h2 className="text-2xl font-bold text-white mb-2">Bem-vindo a bordo! ✈️</h2>
                <p className="text-gray-400 mb-6">Como preferes que te chamemos?</p>
                <form onSubmit={handleSaveName}>
                    <input type="text" required placeholder="Ex: Comandante Silva, Nuno..." className="w-full bg-black border border-gray-600 rounded-xl p-4 text-white text-lg focus:border-blue-500 outline-none text-center mb-6" value={newName} onChange={(e) => setNewName(e.target.value)} />
                    <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-xl transition">Confirmar e Descolar 🚀</button>
                </form>
            </div>
        </div>
      )}
      ...
      <div className={`fixed inset-y-0 left-0 z-50 w-72 bg-gray-900 border-r border-gray-800 transform transition-transform duration-300 ease-in-out shadow-2xl ${isMenuOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="p-6 flex justify-between items-center border-b border-gray-800">
            <h2 className="text-xl font-bold tracking-tight">InFlight <span className="text-blue-500">{t.menuTitle}</span></h2>
        ...
        <nav className="p-4 space-y-2">
            ...
            <div className="border-t border-gray-800 my-4 pt-4">
                ...
                <button onClick={() => setIsMuted(!isMuted)} className={`w-full flex items-center gap-3 p-3 rounded-lg transition ${isMuted ? 'text-gray-500 hover:bg-gray-800' : 'text-green-400 hover:bg-green-900/20'}`}>
                    {isMuted ? <VolumeX size={20} /> : <Volume2 size={20} />} <span>{isMuted ? t.soundOff : t.soundOn}</span>
                </button>
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden relative">
        <div className="p-6 lg:p-10 pb-0 flex items-center gap-4 z-10 bg-black/90 backdrop-blur-sm">
            <button onClick={() => setIsMenuOpen(true)} className="p-2 bg-gray-900 rounded-lg border border-gray-700 hover:bg-gray-800 transition hover:scale-105 active:scale-95"><Menu /></button>
            <div>
                <h1 className="text-3xl font-bold">
                    {view === "home" && `${t.greeting}, ${displayName} 👋`}
                    {view === "search" && `${t.menuRadar} 📡`}
                    {view === "airport" && `${t.menuAirport} 🏢`}
                    {view === "maps" && `${t.mapsTitle} 🧭`}
                </h1>
            ...
            {view === "maps" && (
                <div className="animate-fade-in flex flex-col items-center w-full">
                    <div className="max-w-6xl w-full grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="lg:col-span-2 bg-black border border-gray-800 rounded-2xl p-4 relative overflow-hidden flex items-center justify-center group min-h-[500px]">
                            <div className="absolute inset-0 opacity-20" style={{ backgroundImage: 'linear-gradient(#333 1px, transparent 1px), linear-gradient(90deg, #333 1px, transparent 1px)', backgroundSize: '40px 40px' }}></div>
                            <div className="absolute top-4 left-4 bg-black/70 backdrop-blur px-3 py-1 rounded-full text-xs font-bold border border-gray-700 z-10 text-green-400 flex gap-2 items-center"><Target size={12}/> Live Blueprint</div>

                            <div className="relative w-full max-w-lg aspect-[4/3]">
                                <svg className="w-full h-full" viewBox="0 0 100 100">
                                    <path d="M0,40 L40,40 L40,20 L100,20 L100,40 L40,40 L40,60 L100,60 L100,80 L40,80 L40,60 L0,60 Z" fill="none" stroke="#334155" strokeWidth="2" />
                                    <rect x="2" y="42" width="22" height="16" fill="#1e293b" opacity="0.5" rx="2" />
                                    <rect x="25" y="42" width="8" height="16" fill="#334155" opacity="0.8" rx="1" />
                                    <rect x="42" y="25" width="56" height="10" fill="#1e293b" opacity="0.5" rx="2" />
                                    <rect x="42" y="65" width="56" height="10" fill="#1e293b" opacity="0.5" rx="2" />
                                    <circle cx="38" cy="50" r="10" fill="#0f172a" stroke="#3b82f6" strokeWidth="0.5" />
                                    <text x="38" y="51" fontSize="2" fill="white" textAnchor="middle" fontWeight="bold">Duty Free</text>
                                    <rect x="45" y="88" width="10" height="10" fill="#4c1d95" opacity="0.8" rx="2" />
                                    <text x="50" y="94" fontSize="2" fill="white" textAnchor="middle">T2</text>

                                    {Object.keys(allLocations.graph).map((key) => {
                                        const node = allLocations.graph[key];
                                        const isRelevant = key === startPoint || key === endPoint || (routePath?.type === "transfer" && key === "ShuttleStop");
                                        if (!isRelevant) return null;

                                        return (
                                            <g key={key} className="animate-fade-in">
                                                <circle cx={node.x} cy={node.y} r="2" fill={key === "ShuttleStop" ? "#eab308" : (startPoint === key ? "#22c55e" : "#ef4444")} className="drop-shadow-[0_0_8px_rgba(255,255,255,0.8)]" />
                                                <text x={node.x} y={node.y - 5} fontSize="3" fill="white" textAnchor="middle" className="font-bold drop-shadow-md bg-black/70 px-2 py-1 rounded">{node.label}</text>
                                            </g>
                                        )
                                    })}
                                    ...
                                    {routePath && (
                                        <>
                                            <path d={routePath.pathString} fill="none" stroke={routePath.type === "transfer" ? "#eab308" : "#3b82f6"} strokeWidth="1" strokeDasharray="2,1" className="animate-pulse" />
                                            {routePath.type === "transfer" ? (
                                                <circle r="1.5" fill="#fbbf24"><animateMotion dur="4s" repeatCount="indefinite" path={routePath.pathString} rotate="auto" /></circle>
                                            ) : (
                                                <g>
                                                    <polygon points="-1.5,-1.5 2,0 -1.5,1.5" fill="#00ffff" />
                                                    <animateMotion dur="4s" repeatCount="indefinite" path={routePath.pathString} rotate="auto" />
                                                </g>
                                            )}
                                        </>
                                    )}
